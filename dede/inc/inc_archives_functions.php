...
function AnalyseHtmlBody($body,&$description,&$litpic,&$keywords,$dtype='')
{
	...
	if($description=='' && $cfg_auot_description>0)
	{
		...
		$description = t_addslashes($description);
	}
	//自动获取缩略图
	...
	if($autokey==1 && $keywords=='')
	{
		...
		if(is_array($allindexs) && is_array($titleindexs))
		{
			...
			{
				if(mb_strlen($keywords.$k,'utf-8')>=30)
				{
					break;
				}
				else
		...
		$sp->Clear();
		$sp = null;
		$keywords = $cfg_soft_lang == 'utf-8' ? t_addslashes(gb2utf8($keywords)) : t_addslashes($keywords);
	}
	$body = GetFieldValueA($body,$dtype,$arcID);
	$body = t_addslashes($body);
	return $body;
}
